...
sudo echo "127.0.0.1 gitlab.localhost.com" >> /etc/hosts
...
while true; do
  ...
  total_pods=$(sudo kubectl get pods -n gitlab --no-headers | wc -l)
  running_pods=$(sudo kubectl get pods -n gitlab --no-headers | grep -iE "Running|Completed" | wc -l)
...
while true; do
  sudo kubectl get pods -n argocd -l app.kubernetes.io/name=argocd-server | grep -i "Running" && break
